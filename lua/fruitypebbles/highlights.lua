local c = require("fruitypebbles.colors")
local cfg = vim.g.fruitypebbles_config
local util = require("fruitypebbles.util")
...
hl.syntax = {
	...
	Identifier = { fg = colors.Orange, fmt = cfg.code_style.variables },
...
function M.setup()
		...
		if color_name:sub(1, 1) == "$" then
			...
			if not color_name then
				vim.schedule(function()
					vim.notify(
						'fruitypebbles.nvim: unknown color "' .. name .. '"',
						vim.log.levels.ERROR,
						{ title = "fruitypebbles.nvim" }
					)
				end)
				return ""
			end
	...
	for group_name, group_settings in pairs(vim.g.fruitypebbles_config.highlights) do
		vim.api.nvim_command(
			string.format(
				"highlight %s %s %s %s %s",
				group_name,
